...
function login_check($name,$pass,$host,$username,$password,$dbname){
	...
	if($stmt = $conn->prepare("UPDATE users SET attempts=(?),time_exp=(?) WHERE name=(?)")){
	...
	if($stmt = $conn->prepare("SELECT name,hash,attempts,time_exp FROM users WHERE name=(?)")){
